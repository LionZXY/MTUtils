...
public class MTUtilsMod {
	...
	public void preInit(FMLPreInitializationEvent event) {
		configuration = new Configuration(new File(Loader.instance().getConfigDir() + "/MTUtils", "general.cfg"),
				"1.5");
		configuration.load();
	...
	public void serverLoad(FMLServerStartingEvent pEvent) {
		if (Loader.isModLoaded(CS.ModIDs.GAPI)) {
			pEvent.registerServerCommand(new GTListCommand());
		}
	}
